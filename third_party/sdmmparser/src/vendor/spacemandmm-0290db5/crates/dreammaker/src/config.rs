...
impl Config {
    ...
    pub fn read_toml(path: &Path) -> Result<Config, Error> {
        ...
        Self::read_toml_with_bytes(path).map(|(config, _)| config)
    }

    // APHELION EDIT ADDITION START - ENVIRONMENT INPUT TRACE
    pub(crate) fn read_toml_with_bytes(path: &Path) -> Result<(Config, Vec<u8>), Error> {
        let mut file = File::open(path)?;
        let mut config_toml = String::new();
        file.read_to_string(&mut config_toml)?;
        let config = toml::from_str(&config_toml)?;
        Ok((config, config_toml.into_bytes()))
    }
