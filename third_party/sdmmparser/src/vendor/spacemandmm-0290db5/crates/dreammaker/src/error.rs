...
// APHELION EDIT ADDITION START - ENVIRONMENT INPUT TRACE
use crate::source_trace::{SourceProbeKind, SourceTrace};
// APHELION EDIT ADDITION END
...
pub struct Context {
    ...
    // APHELION EDIT ADDITION START - ENVIRONMENT INPUT TRACE
    source_trace: RefCell<Option<SourceTrace>>,
    source_trace_enabled: Cell<bool>,
    // APHELION EDIT ADDITION END
}
...
impl Context {
    ...
    // APHELION EDIT ADDITION START - ENVIRONMENT INPUT TRACE
    /// Enable per-context capture of parser source reads and filesystem probes.
    ///
    /// Call this on a fresh context, before configuration or parsing. The
    /// default context remains untraced. Calling this more than once keeps the
    /// current trace instead of discarding recorded events.
    pub fn enable_source_trace(&self) {
        if !self.source_trace_enabled.get() {
            *self.source_trace.borrow_mut() = Some(SourceTrace::new());
            self.source_trace_enabled.set(true);
        }
    }

    /// Take the trace captured after enabling it, if tracing was enabled.
    pub fn take_source_trace(&self) -> Option<SourceTrace> {
        self.source_trace_enabled.set(false);
        self.source_trace.borrow_mut().take()
    }

    pub(crate) fn record_source_read(&self, path: &Path, bytes: &[u8]) {
        if self.source_trace_enabled.get() {
            if let Some(trace) = self.source_trace.borrow_mut().as_mut() {
                trace.record_read(path, bytes);
            }
        }
    }

    pub(crate) fn record_source_open_only(&self, path: &Path) {
        if self.source_trace_enabled.get() {
            if let Some(trace) = self.source_trace.borrow_mut().as_mut() {
                trace.record_open_only(path);
            }
        }
    }

    pub(crate) fn source_path_exists(&self, kind: SourceProbeKind, path: &Path) -> bool {
        let exists = path.exists();
        if self.source_trace_enabled.get() {
            if let Some(trace) = self.source_trace.borrow_mut().as_mut() {
                trace.record_probe(kind, path, exists);
            }
        }
        exists
    }

    fn set_source_config_mode(&self, mode: &str) {
        if self.source_trace_enabled.get() {
            if let Some(trace) = self.source_trace.borrow_mut().as_mut() {
                trace.set_config_mode(mode);
            }
        }
    }

    pub(crate) fn mark_source_trace_incomplete(&self) {
        if self.source_trace_enabled.get() {
            if let Some(trace) = self.source_trace.borrow_mut().as_mut() {
    ...
    /* APHELION EDIT REMOVAL START - ENVIRONMENT INPUT TRACE
    ...
    pub fn force_config(&mut self, toml: &Path) {
        // APHELION EDIT ADDITION START - ENVIRONMENT INPUT TRACE
        self.set_source_config_mode("forced");
        self.load_config(toml);
    }

    fn load_config(&mut self, toml: &Path) {
        match Config::read_toml_with_bytes(toml) {
            Ok((config, bytes)) => {
                self.record_source_read(toml, &bytes);
                self.config = config;
            }
            Err(io_error) => {
                self.mark_source_trace_incomplete();
                let file = self.register_file(toml);
                let (line, column) = io_error.line_col().unwrap_or((1, 1));
                DMError::new(
    ...
    pub fn autodetect_config(&mut self, dme: &Path) {
        let toml = dme.parent().unwrap().join("SpacemanDMM.toml");
        self.set_source_config_mode("autodetect");
        if self.source_path_exists(SourceProbeKind::ConfigDiscovery, &toml) {
            self.load_config(&toml);
        }
    }
    // APHELION EDIT ADDITION END
