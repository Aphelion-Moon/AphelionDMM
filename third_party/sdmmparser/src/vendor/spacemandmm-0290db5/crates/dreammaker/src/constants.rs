                ...
                "fexists" if self.defines.is_some() && self.context.is_some() => {
                    ...
                    match args[0].as_term() {
                        Some(Term::String(passed_path)) => {
                            ...
                            // APHELION EDIT CHANGE - ENVIRONMENT INPUT TRACE - ORIGINAL: current_dir.join(passed_path).exists().into()
                            self.context.unwrap(/* is_some checked above */).source_path_exists(
                                SourceProbeKind::FExists,
                                &current_dir.join(passed_path),
                            ).into()
                        }
                        _ => {
                            return Err(self
