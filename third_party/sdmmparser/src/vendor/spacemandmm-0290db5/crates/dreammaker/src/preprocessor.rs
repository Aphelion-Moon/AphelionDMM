...
// APHELION EDIT ADDITION START - ENVIRONMENT INPUT TRACE
use super::SourceProbeKind;
...
impl<'ctx> Preprocessor<'ctx> {
    ...
    pub fn from_buffer<S: Into<Cow<'ctx, str>>>(
        context: &'ctx Context,
        env_file: PathBuf,
        buffer: S,
    ) -> Self {
        // APHELION EDIT ADDITION START - ENVIRONMENT INPUT TRACE
        // The supplied root contents do not come from a replayable file read.
        context.mark_source_trace_incomplete();
        // APHELION EDIT ADDITION END
    ...
    */
    ...
    pub fn push_file<R: io::Read + 'static>(
        &mut self,
        path: PathBuf,
        read: R,
    ) -> Result<FileId, DMError> {
        // APHELION EDIT ADDITION START - ENVIRONMENT INPUT TRACE
        // This public API accepts an arbitrary reader, so its path cannot
        // guarantee replay of the supplied bytes.
        self.context.mark_source_trace_incomplete();
        // APHELION EDIT ADDITION END
        let idx = self.context.register_file(&path);
        self.include_stack.stack.push(Include::File {
            // APHELION EDIT ADDITION START - ENVIRONMENT INPUT TRACE
            // APHELION EDIT CHANGE - ENVIRONMENT INPUT TRACE - ORIGINAL: lexer: Lexer::from_read(self.context, idx, read)?,
            lexer: Lexer::from_read_at(self.context, idx, &path, read)?,
            // APHELION EDIT ADDITION END
            //file: idx,
            path,
        });
    ...
    fn prepare_include_file(&mut self, path: PathBuf) -> Result<Include<'ctx>, DMError> {
        ...
        // APHELION EDIT CHANGE - ENVIRONMENT INPUT TRACE - ORIGINAL: let read = io::BufReader::new(File::open(&path).map_err(|e| DMError::new(self.last_input_loc, format!("failed to open file: #include {path:?}")).with_cause(e))?);
        ...
        let register = path
            .strip_prefix(self.env_file.parent().unwrap())
            .unwrap_or(&path);
        ...
        if let Some(&loc) = self.include_locations.get(&file_id) {
            if !self.multiple_locations.contains_key(&file_id) {
                // APHELION EDIT ADDITION START - ENVIRONMENT INPUT TRACE
                self.context.record_source_open_only(&path);
                // APHELION EDIT ADDITION END
                Err(
                ...
                // APHELION EDIT ADDITION START - ENVIRONMENT INPUT TRACE
                // APHELION EDIT CHANGE - ENVIRONMENT INPUT TRACE - ORIGINAL: lexer: Lexer::from_read(self.context, file_id, read)?,
                let lexer = Lexer::from_read_at(self.context, file_id, &path, read)?;
                // APHELION EDIT ADDITION END
                Ok(Include::File {
                    path,
                    //file: file_id,
    ...
    fn push_output(&mut self, token: Token) {
        self.output
            .push_back(LocatedToken::new(self.last_input_loc, token));
    }
        ...
        match read {
            Token::Punct(Punctuation::Hash) => {
                ...
                match &ident[..] {
                    ...
                    "endif" => {
                        self.pop_ifdef()
                            .ok_or_else(|| DMError::new(self.last_input_loc, "unmatched #endif"))?;
                    }
                    "else" => {
                        let last = self
                            .pop_ifdef()
                            .ok_or_else(|| DMError::new(self.last_input_loc, "unmatched #else"))?;
                    ...
                    "elif" => {
                        let last = self
                            .pop_ifdef()
                            .ok_or_else(|| DMError::new(self.last_input_loc, "unmatched #elif"))?;
                        let enabled = self.evaluate();
                        self.ifdef_stack
                            .push(last.else_if(self.last_input_loc, enabled));
                    }
                    // --------------------------------------------------------
                    ...
                    "include" => {
                        ...
                        for candidate in [
                            ...
                            // APHELION EDIT CHANGE - ENVIRONMENT INPUT TRACE - ORIGINAL: if !candidate.exists() {
                            if !self
                                .context
                                .source_path_exists(SourceProbeKind::IncludeCandidate, &candidate)
                            {
                                continue;
                            }
                        ...
                        self.context.register_error(DMError::new(
                            self.last_input_loc,
                            format!("failed to find #include {path:?}"),
                        ));
                        return Ok(());
                    }
                    ...
                    "define" => {
                        ...
                        if let Some(annotations) = self.annotations.as_mut() {
                            annotations.insert(
                                define_name_loc
                                    ..define_name_loc.add_columns(define_name.len() as u16),
            ...
            Token::Ident(ref ident, whitespace) => {
                ...
                if self.in_interp_string > 0 {
                    self.danger_idents
                        .insert(ident.clone(), self.last_input_loc);
                }
        ...
        loop {
            ...
            if let Some(tok) = self.inner_next() {
                ...
                while let Some(ifdef) = self.pop_ifdef() {
                    self.context
                        .register_error(DMError::new(ifdef.location, "unterminated #if/#ifdef"));
                }
                return None;
            }
        }
